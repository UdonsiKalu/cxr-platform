...
Do **not** duplicate full app CI in ops-lab. **`build-k8-images`** always lints Helm charts on push. Full **`docker/analyzer/Dockerfile`** build on GHA is **optional** — analyzer source is local (`cxrlabs-dev/claim_analysis_tools`), not in this repo.
...
Phased demo runbook: **`gitops-phase-demo.md`**
